"""
   ...
   Description:   Interacts with EconGuide.py, builds the GUI for adding uncertainties to non-disaster benefits,
                  the page for the user to input Non D Benefits uncertainty distributions.
"""
...
class NonDBensUncertaintiesPage(tk.Frame):
    ...
    def __init__(self, parent, controller, data_cont_list):
        ...
        ben_lbl = tk.Label(self, text="Non Disaster Related Benefits Uncertainties", font=LARGE_FONT)
    ...
    def show_info(self):
        ...
        messagebox.showinfo("More Information",
                            ...
                            "estimate defined by the user for the non-disaster related benefit.\n"
    ...
    def add_uncertainty(self, moveon=False):
        ...
        for plan in self.data_cont.plan_list:
            for ben in plan.nond_bens.indiv:
                new_values = []
                for entry in self.ranges[plan.id_assign][plan.nond_bens.indiv.index(ben)]:
                    new_values.append(entry.get())
                ben.add_uncertainty(new_values,
                                    self.choices[plan.id_assign][plan.nond_bens.indiv.index(ben)].get())
    ...
    def check_page(self, printout=True):
        ...
        for plan in self.data_cont.plan_list:
            for ben in plan.nond_bens.indiv:
                dist = self.choices[plan.id_assign][plan.nond_bens.indiv.index(ben)].get()
                nums = self.ranges[plan.id_assign][plan.nond_bens.indiv.index(ben)]
                try:
