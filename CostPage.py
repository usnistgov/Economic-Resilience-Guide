...
#        self.scrollbar.grid_configure(group3)
    ...
    def check_page(self, printout=True):
        ...
        for choice in self.choices:
            if (choice)[:len(self.title_ent.get())] == self.title_ent.get():
                if self.b_bool.get() and " - <Base Plan>" in choice:
    ...
    def copy_cost(self):
        ...
        self.cost_ent.insert(tk.END, cost_amount)
        self.desc_ent.delete('1.0', tk.END)
        self.desc_ent.insert(tk.END, cost_desc)
        self.choice.set(cost_type)
        if cost_type == "Omr":
            self.omr_selection.set(this_omr_type)
            self.year_start_ent.delete(0, tk.END)
            self.year_start_ent.insert(tk.END, omr_year)
            if this_omr_type == "Recurring":
                self.year_rate_ent.delete(0, tk.END)
                self.year_rate_ent.insert(tk.END, omr_rec)
